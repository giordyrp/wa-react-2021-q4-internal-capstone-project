    ...
    <Section title="Cart">
      {loading ? <Spinner /> : cart.length === 0 ? (
        <FlexDiv align="center" direction="column">
          ...
          <Button color="primary" onClick={() => history.push('/products')}>View Products</Button>
        </FlexDiv>
      ) : (
        <>
        <Styled.StyledCard>
          ...
          <Row>
            ...
            <Col xs={2}>
              <p>
                ${cart.reduce((acc, product) => acc + product.count * product.data.data.price, 0)}
              </p>
            </Col>
          </Row>
          </Styled.StyledCard>
          <FlexDiv justify="flex-end">
            <Button color="primary" size="large" onClick={() => history.push('/checkout')}>Proceed to Checkout</Button>
          </FlexDiv>
        </>
      )}
    </Section>
